...
//       // Data set karte waqt safe check lagayen
...
//       toast.error('Data sync fail ho gaya. Console check karein.');
//     } finally {
//       setLoading(false);
//     }
//   };
...
const AdminDashboard = () => {
  ...
  const handleStatusUpdate = async (donationId, newStatus) => {
    const toastId = toast.loading('Updating status...');
    try {
      // Backend route updated to .put for status
      await donationAPI.updateStatus(donationId, newStatus);
      toast.success('Donation Verified!', { id: toastId });
      fetchData();
    } catch (error) {
  ...
  // Stats Card Config for better readability
  const statsConfig = [
    { label: 'Total Donations', val: stats?.totalDonations || 0, color: 'emerald', icon: HandHeart },
    { label: 'Total Revenue', val: `PKR ${(stats?.totalAmount || 0).toLocaleString()}`, color: 'blue', icon: TrendingUp },
    { label: 'Verified Funds', val: `PKR ${(stats?.verifiedAmount || 0).toLocaleString()}`, color: 'green', icon: CheckCircle2 },
    { label: 'Pending Verification', val: `PKR ${(stats?.pendingAmount || 0).toLocaleString()}`, color: 'amber', icon: AlertCircle }
  ];
          ...
          >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {donations.slice(0, 6).map((d) => (
                          <tr key={d._id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-8 py-4 font-bold text-gray-700">{d.user?.name || 'Unknown User'}</td>
                            <td className="px-8 py-4 font-black text-emerald-700">PKR {d.amount.toLocaleString()}</td>
                            <td className="px-8 py-4 text-sm text-gray-500 font-medium">{d.type}</td>
                            <td className="px-8 py-4 text-xs font-black uppercase text-emerald-600">{d.status}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {donations.map((d) => (
                        <tr key={d._id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-8 py-5 font-bold">{d.user?.name}</td>
                          <td className="px-8 py-5 font-black text-emerald-700">PKR {d.amount.toLocaleString()}</td>
                          <td className="px-8 py-5 text-xs font-bold text-amber-600">{d.status}</td>
                          <td className="px-8 py-5">
                            {d.status === 'Pending' && (
                              <button
                                onClick={() => handleStatusUpdate(d._id, 'Verified')}
            ...
            {activeTab === 'campaigns' && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Campaign Form & List logic remains same as provided in your snippet */}
